...
if (NT~=NP)
    disp(['ERROR AirProperties: array size mismatch']); 
    return;
end
...
for (j=1:NT)
    mu(j) = interp1(T_vec,mu_vec,T(j));  %dynamic viscosity
    k(j) = interp1(T_vec,k_vec,T(j));    %thermal conductivity
    Cp(j) = interp1(T_vec,Cp_vec,T(j));  %specific heat
end
